...
export function UnassignedQueue({ id, onStatsChanged }: UnassignedQueueProps) {
  ...
  return (
    <div id={id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-4">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-100 disabled:text-slate-400 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-xs transition cursor-pointer"
          >
            {runningAuto ? (
        ...
        {resultsLog && (
          <div className="mb-4 bg-slate-50 border border-slate-200 rounded-lg p-3 relative text-xs">
            ...
            <div className="space-y-1 max-h-[140px] overflow-y-auto divide-y divide-slate-100 font-sans text-slate-600">
              {resultsLog.length > 0 ? (
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            ...
            <span className="text-xs text-slate-400 font-sans">Loading incoming queue...</span>
          </div>
        ) : unassigned.length > 0 ? (
            ...
            {unassigned.map(emp => (
              ...
              >
                <div className="pr-2 truncate">
                  <div className="font-bold text-slate-900 leading-tight truncate">{emp.name}</div>
                  <div className="text-[10px] text-slate-400 font-mono mt-0.5">{emp.id} • {emp.role}</div>
                  <div className="text-[10px] text-slate-400 font-sans mt-0.5">Joined: {emp.joinDate}</div>
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <span className="text-[10px] bg-slate-200/60 text-slate-600 border border-slate-300/40 px-1.5 py-0.5 rounded font-bold">
          ...
          <div className="bg-slate-50 border border-dashed border-slate-200 rounded-lg p-8 flex flex-col items-center text-center">
            ...
            <h5 className="font-bold text-slate-700 text-xs">Queue Cleared</h5>
      ...
      <div className="text-[10px] text-slate-400 text-center font-sans mt-4 pt-2 border-t border-slate-100">
