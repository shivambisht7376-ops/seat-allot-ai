...
export function ProjectManager({ id, projects, onProjectCreated, onStatsChanged }: ProjectManagerProps) {
  ...
  const [currentTeamList, setCurrentTeamList] = useState<Employee[]>([]);
  ...
  const loadTeamAndUnassigned = async (projCode: string, search = '') => {
    try {
      const [teamRes, unassignedRes] = await Promise.all([
        fetch(`/api/employees?limit=500&projectCode=${projCode}`, { headers: authHeader as any }),
        fetch(`/api/employees?limit=50&isUnassigned=true&textSearch=${search}`, { headers: authHeader as any })
      ]);
      
      if (teamRes.ok) {
        const teamData = await teamRes.json();
        setCurrentTeamList(teamData.data);
      }
      if (unassignedRes.ok) {
        const unassignedData = await unassignedRes.json();
        setUnassignedList(unassignedData.data);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  useEffect(() => {
    if (managingTeamProject) {
      loadTeamAndUnassigned(managingTeamProject.code, unassignedSearch);
    }
  ...
  const handleRemoveMember = async (empId: string) => {
    ...
    try {
      await fetch(`/api/employees/${empId}/assign-project`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(authHeader as any) },
        body: JSON.stringify({ projectCode: '' }) // Remove from project
      });
      loadTeamAndUnassigned(managingTeamProject!.code, unassignedSearch);
      onStatsChanged();
      onProjectCreated();
                    ...
                    currentTeamList.map(emp => (
                      <div key={emp.id} className="flex items-center justify-between p-2 bg-white rounded border border-slate-200">
                        ...
                        <button
                          ...
                          className="text-[10px] text-rose-600 font-bold px-2 py-1 bg-rose-50 hover:bg-rose-100 rounded transition"
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="relative mb-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg py-1.5 pl-9 pr-3 text-xs focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                <div className="flex-1 overflow-y-auto border border-slate-100 rounded-lg bg-slate-50/50 p-2 space-y-1">
                ...
                <button 
                  ...
                  className="w-full mt-3 py-2 bg-indigo-600 text-white rounded-lg font-bold flex items-center justify-center gap-2 disabled:opacity-50 text-xs"
                >
